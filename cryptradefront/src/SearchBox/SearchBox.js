...
const SearchBox = ({handleSearchClick}) => {
  ...
  useEffect(() => {
    const fetchTopCoins = async () => {
        try {
        const response = await axios.get('http://localhost:5024/api/Crypto/get-sorted-by-price-data');
  ...
  useEffect(() => {
    ...
    const fetchSearchResults = async () => {
        try {
            const response = await axios.get(`http://localhost:5024/api/Crypto/search-crypto?searchTerm=${searchTerm}`);
